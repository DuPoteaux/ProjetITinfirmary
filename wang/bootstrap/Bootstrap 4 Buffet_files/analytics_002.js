function paddleAnalyticsClient(stub) {
    ...
    // Name of cookie 
    this._cookieName = "_calq_d"; // To be changed to '_paddleAnalytics_d' at some point to avoid conflict. (We override this to 'analyticsCookie' in Paddle.js anyway - so it shouldn't matter too much).
    // Optional domain to use when writing cookies (if null, will be omitted)
    this._cookieDomain = null;
    // Flag to indicate if we only want to track identified users
    ...
    this.action = {
        ...
        // Reference back to paddleAnalyticsClient from track
        _client: this,
    ...
    this.user = {
        ...
        // Reference back to paddleAnalyticsClient from user
        _client: this,
        ...
        clear: function () {
            ...
            this._client.action._hasTracked = false; // New user id, nothing tracked
    ...
    this._ajaxPost = function (url, params, success, error, blocking) {
        if (this._writeKey == null) {
            ...
            if (!this._browserInfo.cors) {
                ...
                // Blocking requests were used for items that might happen during offload (e.g. trackLinks). We use Beacon API if possible
                blocking = blocking != null ? blocking : false;
                if (blocking && navigator.sendBeacon) {
                    this._beaconPost(url, params, success, error);
                } else {
                    // Non blocking, or old browser that still supports async and not beacon
    ...
    /// <summary>
    /// Submits data using the Beacon API (doesn't check for support).
    /// </summary>
    this._xmlHttpPost = function (url, params, success, error, blocking) {
        ...
        xmlHttp.onreadystatechange = function () {
            if (xmlHttp.readyState === 4 /* XMLHttpRequest.DONE */) {
                ...
                if (xmlHttp.status === 200 && response != null && response.error == null) {
                    ...
                    // If this was an internal server error we can retry it later
        ...
        // If we timeout then we can also retry
        if (!blocking) {
        ...
        // Must call send last as IE11 other property changes if sent
    ...
    /// <summary>
    /// Offline queue to handle requests if the device is offline (or something bad has happened and the API is offline).
    /// </summary>
    this._offlineQueue = {

        /// <summary>
        /// The name of the key we use for offline storage.
        /// </summary
        ...
        enqueueApi: function (url, apiParams) {
            // Grab queue data and add to end
            var queue = this._getQueueData();
            queue.push({ url: url, payload: apiParams, timestamp: new Date().getTime() });
            // Write back to queue
            localStorage.setItem(this._storageKey, JSON.stringify(queue));
        },

        /// <summary>
        /// Removes the first item from the queue. Will return null if no items available.
        /// </summary>
        dequeueApi: function (peek) {
            ...
            if (queue.length > 0) {
                ...
                // Need to write back queue with item removed (unless peeking)
                if (!peek) {
                    localStorage.setItem(this._storageKey, JSON.stringify(queue));
                }
                return item;
        ...
        /// <summary>
        /// Peeks the first item from the queue. Will return null if no items available.
        /// </summary>
        peekApi: function () {
        ...
        /// <summary>
        /// Gets the queue from local storage. 
        /// </summary>
        ...
        /// <summary>
        /// Gets if queueing is actually supported by this device.
        /// </summary>
        isSupported: function () {
            try {
                var known = (typeof (Storage) !== "undefined");
                var test = known && window.localStorage.getItem("test"); // See if SecurityError thrown (user disabled)
                return known;
            } catch (err) {
                return false;
            }
    ...
    /// <summary>
    /// Flushes the queue (i.e. sends queued data to the API if possible).
    /// </summary>
    this._flushOfflineQueue = function () {
        if (this._offlineQueue.isSupported()) {
            var peekApi = null;
            var flushIfBefore = (new Date().getTime() - (60 * 1000));   // More than 60s old
            var ignoreIfBefore = (new Date().getTime() - (14 * 24 * 60 * 60 * 1000));   // More than 2 week old
            while (peekApi = this._offlineQueue.peekApi(true)) {
                // Check timestamp on this entry. If too recent, stop flushing
                if (peekApi.timestamp > flushIfBefore) {
                    break; // Too recent
                } else {
                    var queuedApi = this._offlineQueue.dequeueApi(); // Pull this item now (we only peeked before)
    ...
    this._overwriteStub = function (stub) {
        ...
        if (stub.writeKey == null) {
            ...
            this.init = function () {
                ...
                window.paddleAnalytics = new paddleAnalyticsClient(stub);  // Overwrite with stub as normal now init called
            }
        } else {
            ...
            // If we have any queued items from a previous session we can flush them too
    ...
    ///
    ///     * cookieName - Overrrides the cookie name this client library is using to persist data. Default is "_paddleAnalytics_d"
    ...
    ///     * identifiedOnly - Sets whether to track data for everyone (the default), or only for identified users only.
    ///
    /// </summary>
